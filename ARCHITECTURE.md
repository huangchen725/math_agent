...
普通 `ReasoningAgent(client)` 使用 `deployment_policy()`。原题与metadata校验后建立唯一ExecutionBudget；metadata不进入模型消息。`_v2_solve` 首先调用语料模块的 `build_evidence_plan`，返回整题身份匹配、仅路线一可用的题答参考、两条路线可用的方法资料及数值诊断。所有资源均为可选，缺失/损坏记录不可用并进入模型求解。

严格同题快答仍需一次公开模型完整核对和答案保真检查，完整题面的数学反证可否决它。未提前结束的题目进入两条依次派发且信息独立的路线：第二条只接收原题和方法，不读取第一条答案或题答参考。每条路线最多提交4个声明式计算；必要时把实际结果回灌一次，保留原候选和修正候选分别记录。证明题使用目标、条件、方法、关键证明义务及缺口；这些均是模型陈述，不能自动获得精确证明资格。

模型只通过普通文本中的 `<solver_plan>` 请求计算，平台client不接受tools扩展。白名单执行器支持15类运算：精确有理数、多项式、微分、代入、候选根检查、多重定积分、矩阵行列式/秩/线性方程/特征对、有限求和、二项式、模幂、gcd/lcm及整数区间根枚举。限制3变量、128项、总次数24、系数256bit、20000计数操作、8×8矩阵、256枚举点；输入形状、递归、字符串和结果长度均受限。解析只构造有界数学节点，不执行模型Python代码。子任务积分可以计算有向界限；整题区域积分证明额外检查仿射嵌套区域合法性，不能混淆两种语义。

完整题面绑定的数学pass优先、fail排除；其余候选先考虑是否有被实际计算否定的步骤，再考虑带具体理由的模型审查、条件/小问遗漏和未解决证明义务。模型审查是软证据；unknown不清空候选。审查若申请计算，必须看到实际结果后再形成选择，最多追加一次审查。明确缺陷可触发一次定向修补；修补只给出计算计划时，实际结果允许回灌一次完成终答。expected只在可证明不同的表示下标False，未知格式不作为数学反证。所有调用共用原预算；通常采用4096解题、3072计算后补全、2048审查，均不超过配置及8192硬限。每题最多16次请求及原计算/token/600秒预算；剩余时间不足45秒或只剩一次请求时停止可选分支。该时间检查不承诺取消已派发的未知client请求。

公开trace只含静态阶段标识、白名单数值统计和预算，不包含题面、参考、模型正文或错误内容。最终交付保留单位、上下标、小问顺序和适用条件；末行只有一个 `最终答案：`。任何后续传输或协议故障停止派发，保留已有完整且未被整题反证的候选，不重新启动旧流程。`legacy_deployment_policy()` 和显式全关闭 `Q1Policy()` 用于历史回归；下面带日期的说明及原流程章节保留为旧策略事实，不定义新默认的候选数和选答方式。

> **2026-09-13 solver-v2（离线验收通过，线上配对未完成）**：用户批准整体替换内部求解流程。普通入口启用 `solver_v2`，同一预算内依次进行受限资料检索、严格同题核验、两条独立读题路线、声明式精确子任务计算、条件与证明义务审查、一次定向修补及证据选答。路线二不读取路线一答案或题答参考；方法资料只提供带前提的理论。新控制器和有界计算仍位于 `user_agent.py`，检索仍位于 `xh202627_corpus.py`；正式六源加载器、公开四参数False、依赖和原硬预算保持。旧 `AgentConfig` 和旧提示保留，`legacy_deployment_policy()` 显式对应1841685策略，`Q1Policy()`仍全关闭。模型结构化题意和子任务计算不得升级为整题数学证明，只有严格完整题面绑定的检查可给出整题pass/fail。新流程传输失败停止后续派发，尽可能交付已完成且未被整题反证的候选。线上及离线验收状态见 `docs/evaluations/SOLVER_V2_20260913.md`；下文日期记录属于历史版本。

> **2026-09-10提交前解析修补**：快答专用协议对CONDITIONS、合并后的CHECK及最终答案body统一检查嵌套字段、围栏与隐藏控制符；末行只直接读取唯一精确前缀后的完整body，不委托通用答案提取器再选择其他标记。任一字段冲突均退出快答并保留同题剩余预算。模型提示、公开四参数、答案一致性、语料根摘要与正式导入闭包不变。重新验收见 `docs/evaluations/PREPUSH_REVIEW_20260910.md`；下方1114项验收对应修补前的dcbe4d0。

> **2026-09-10晚间首日候选（本地验收通过）**：在既有六源闭包内扩展 `xh202627_corpus.py`，通过代码固定摘要验证根manifest、分桶索引与按offset读取的1054条题答；构造不读取全库。完整题面身份经一次公开模型实质核对、完整终答、严格答案一致性与本地数学反证门槛后才允许提前返回。核对unknown/冲突/length退出快答并继续同题剩余预算，不重启solve或重复注入被拒参考。方法参考至多注入一个普通候选；不足两个独立概念、没有共同明确方法/非平凡完整数学片段或函数结构冲突时返回空。每题临时参考用独立ContextVar保存并在finally清除，无跨题答案缓存。`answer_bank_fastpath`、`answer_bank_reference`、`completed_answer_repair`三项默认开启，显式Q1Policy仍全关闭；43已启用四项继续保留。正确答案交付修复仅处理明确stop、唯一完整Final Answer块与整题精确pass。客户端、拓扑、依赖和每题预算不改；详见首日报告。

新库exact与topic共用每题4MiB/2秒检查预算；至多4096 postings、128候选、8正文，参考至多6000字符。完整答案body保留编号/符号/条件，只去锚定英文标题与排版空白；说明文退出快答。模型独立核对通过后逐字采用给定body，输出绕过旧通用格式化以保留度数/下标/小问顺序。CONDITIONS单行、CHECK可有界续行、末MATCH/VERDICT/最终答案字段严格；最多16非空行、CHECK2000字符、响应6000字符，重复字段与未知均拒绝。896条当前具备快答预筛资格，资格不等于数学证明。正式六源均已通过真实资源隔离故障探针；最终两种Python各1114项门禁通过，未获正式分数。

> **2026-09-10 联合候选**：普通 `ReasoningAgent(client)` 现在使用命名 `deployment_policy()`，明确开启 `concise_recovery`、`bounded_math`、`evidence_selection`、`reasoned_verifier`；显式传入 `Q1Policy()` 仍是全关闭实验配置。新路由不受 Q1 开关控制，因此严格历史 baseline 使用 `b87e4b9` 的不可变六源快照，不能仅以全关闭开关冒充旧源码。模型、候选数、温度、四参数 False、8192 单次上限和原预算不变。部署状态由正常构造入口测试，不把代码存在当作功能启用。

`evidence_selection` 在已有合格候选聚合时应用：存在模型明确支持的答案组时，仅移除组内所有候选都只有否定票的整个答案组，再按原同答案票数及置信度聚合。组内存在 pass 或 unknown 时保留；没有任何明确支持时保留原排序。完整任务的精确证明仍先于模型聚合，证明失败的候选不能由多数票恢复。若精确核验否定所有合格候选，最多通过既有 `_reflect` 追加一次定向重算，并重新检查答案；仍为 fail/截断时拒绝，unknown 新候选可交付但不称证明正确。修复共享原每题 16 请求、600 秒、200000 token 预算。

`reasoned_verifier` 使用新的短核算提示，保留 1024 token 和温度 0；输出为 `CHECK: ...` 加单行 `VERDICT: A/B/UNKNOWN`。解析要求完整的有界行结构，拒绝断行裁决、重复裁决、空检查和明确截断；完整旧式 `VERDICT: A/B` 仍兼容。检查文本仅在本题内部与本地授权观察器中使用，公开 trace 不包含它。

新路由在既有 `domain_prompts.py` 内识别双语完整词与数学记号、最长术语和同分弃权；旧领域提示常量未改。最多缓存 8 份固定词表的已编译表达式，不存题面/答案/匹配结果。微积分核验在入口底部实现有界 Q(x,sqrt(P))、精确链式求导与 Sturm 全实轴符号证明，允许有理函数、一个根式及常数倍 ln/atan；完整绑定显式整数参数块、要求 +C。未知条件、多根式、三角/指数、不可证明全实域均 unknown。上限为输入/答案各 1200 字符、256 token、192 AST 节点、16 层组、32 次多项式、1024 bit 系数、20000 操作；不引入 SymPy、动态执行、正式模块、依赖或外网。正式导入闭包和公开 trace 脱敏不变。

> 状态：R1 联合提分候选；从已正式运行的 `b87e4b9` 修改，尚未取得新候选正式分数
>
> 更新日期：2026-09-10
>
...
构造形式是 `ReasoningAgent(client, config=None, *args, local_adapter=None, local_policy=None, **kwargs)`。仅接受此入口真实定义的 `AgentConfig` 为配置；不透明参数和未知 kwargs 不污染配置，也不触发 client 能力探测。有效的显式 `Q1Policy` 可覆盖策略，否则使用当前命名部署配置。未知 client 一律按上述四参数调用 `chat`。本地入口可显式传入 `LocalToolAdapter`，`complete()` 返回 `{response, metadata}`，`run_tools()` 返回文本、工具 trace 与本次 metadata；不存在共享最近响应 getter。
...
1. **领域路由**：`_detect_domain()` 委托既有领域模块做双语完整词、最长术语和数学记号匹配；未匹配或最高分同分使用通用提示。
2. **候选生成**：默认生成 2 个工具来源候选和 1 个纯推理候选，策略温度 `0.6`、单次上限 `8192` tokens；每次公开请求显式发送 `thinking_mode=False`，正式工具来源候选仍走纯文本路径。
...
5. **验证**：每个候选默认由模型验证 1 次，温度 `0.0`，先输出简短 CHECK 核算，再输出完整 VERDICT: A/B/UNKNOWN；忽略纯空白行，拒绝断开/重复标签和显式截断。完整单行 VERDICT: A/B 保持兼容。关闭 reasoned_verifier 的旧实验保留原严格/短标签策略。长候选保留头尾，避免截掉末尾答案；验证结果写入结构化 `Verification`，公开 trace 不保存 CHECK 正文。有可抽取答案的候选仍加 `0.3`，无答案减 `0.5`。模型核算不能替代确定性证明。
