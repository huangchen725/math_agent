...
> 状态：当前生效  
> 基线提交：`d9f7585b4dc19d55d16c1b72faf3cd9d8fb5d3ae`
...
| S1 | `3ba0610` | 消除根目录与旧包的重复实现，把 `user_agent.py` 收缩为兼容门面；增加公开接口与 Demo 安全网；当时物理实现迁入 `math_agent/` | “唯一实现、薄门面、公开类型对象身份一致、Demo 依赖延迟加载”不可回退；物理包目录已被后续兼容回迁取代，不得把扁平部署误写成恢复双实现 |
...
| 保守兼容内核 | `05ec5c2` | 保留模块化架构，把正式路径收敛为 3 个纯文本候选、每候选 1 次模型验证、模型多数票和有界恢复 | 默认恰好 6 次普通三参数调用；tools/critic/reflection/deterministic 旧开关不得启用另一套正式协议；在线恢复仍待正式评测 |
| 根模块兼容回迁 | `d9f7585` + 2026-09-04 修正工作树 | 保留 S2～S4 分层，把全部运行模块从 `math_agent/` 移到仓库根；修复远程提交误伤的包内相对导入、发布白名单和公开诊断泄露；增加只复制根 `.py` 的严格 client 回归 | 不得恢复第二份实现或巨型 Agent；正式部署不得依赖 `math_agent/` 子目录；该措施与官方 0 request 的因果关联仍待线上确认 |

结构阶段的完成不等于数学能力提高。P1 的收益、S2～S6 的工程收益和真实正确率必须分别陈述，不能互相替代。

## 3. 当前策略冻结快照

下列参数是变更控制快照，权威实现位于根目录 `agent_config.py`。任何改动都必须只改变一个实验变量，冻结数据、提交、模型、配置和成本，并补离线回归；没有真实评测授权时不得调用 API。
...
- **ARC-001**：仓库根目录的分层 Python 模块是唯一运行实现；`agent.py` 是 `ReasoningAgent`/`AgentConfig` 的唯一公开类型源，`user_agent.py` 只做基于自身 `__file__` 的同级模块导入引导和兼容重导出。入口必须能在不复制任何包子目录、任意当前目录、隔离解释器中按绝对路径加载并执行；`main.py`、`demo.py`、`verify_math.py` 只做适配。
...
- **ARC-005**：新增、删除或移动根运行模块时必须同步实验 `RUNTIME_FILES` 指纹、发布显式白名单、compileall/Bandit 范围与结构测试，避免实验冻结、扫描或交付漏算实际代码；不得用空前缀或任意根 `*.py` 放宽发布范围。
...
| ARCH-001 | 根实现与包实现曾重复，入口/依赖容易漂移；S1 后的唯一包实现又与连续四次正式 0 request 完全相关 | 早期缺少唯一实现和公开契约测试；后续测试没有模拟只保留根文件的部署形态 | 唯一实现仍保留，物理部署已回迁根模块；**线上关联待验证** | 结构测试阻止第二份实现，并在无 `math_agent/` 的根文件副本中执行严格三参数 `solve()` |
...
| CLIENT-001 | `70ac53f`、`c088a84`、已含三参数投影的 `eb5d8d4` 和保守内核 `05ec5c2` 四次官方评测均为 112/112 error、0 request、0 token；这反证 `thinking_mode/tools`、构造器或保守协议中的任一单项修复足以恢复线上。官方固定 baseline `43be244` + 精确 `eb5d8d4` + fake HTTP 的 112 题对照为 112 success、672 次进入公开 client | 平台未公开注入 client 的类/版本、返回类型、校验顺序与逐题异常；正式环境至少存在一项 baseline 外差异 | 已实现三参数投影、固定 3×1 纯文本兼容内核、私有读取移除、严格 fake 和根模块部署；状态仍为 **未解决** | 下一次先确认实际 SHA、`request_count>0`、`success>0`、`error=0`；向主办方索取脱敏异常类型、精确 client 契约及正式 runner 与公开 baseline 的差异，未取得前不得再写唯一根因 |
| ENTRY-001 | S1 后根入口改为包门面；在仓库根不在 `sys.path` 的官方式绝对路径加载中可复现 `ModuleNotFoundError: math_agent` | 入口测试继承了仓库当前目录，未覆盖 path-based 独立 worker | 先按 `__file__` 引导修复，现已进一步取消正式部署对包子目录的依赖；**线上关联待验证** | 每次正式发布用 `python -I` 从仓库外动态加载根入口，并在只复制根 `.py` 的目录中执行严格三参数 `solve()` |
| ENTRY-003 | 唯一有效官方版本 `350a267` 使用根扁平模块；从 S1 包化进入正式评测的 `70ac53f`、`c088a84`、`eb5d8d4`、`05ec5c2` 均为 0 request。条件性根文件模拟中包版导入失败、扁平版可运行 | 正式 worker 的精确复制/导入机制未公开，相关性和条件性复现不能证明 judge 实际丢弃 `math_agent/`；最新日志显示平台至少先克隆完整仓库 | 已保留分层实现并回迁根模块，删除导入异常正文回显，加入根文件副本回归；**官方因果待验证** | 下一次评测只以绑定 SHA、请求/成功/error 指标验证兼容性；若仍 0 request，停止结构猜测并索取逐题异常与 runner 加载流程 |
...
| OFFICIAL-002 | 连续四次 112 error / 0 request 暴露 client/response/error/runner 契约缺失；模型 ID、预算、资源、Judger、工具和变更版本同样未完整发布。官方固定 baseline 与 `eb5d8d4` 的 112 题精确入口对照全部 success，证明正式平台至少存在一项未公开差异 | 把 baseline 示例、宽松 fake、历史成功、结构相关性或一次候选缺陷修复误当平台实现保证；反过来把正式失败直接归因于某一未公开机制也不成立 | **未解决**；已登记十类 `OFFICIAL-GAP-*` 并采用最小接口、本地预算、根模块入口和外层契约保护 | 只有满足对应缺口所需的精确书面证据才能关闭；正式运行前向主办方请求正式 runner 与公开 baseline 的差异、实际构造参数、client 版本/签名、逐题异常类型、状态分类和计数语义 |
...
| COMPAT-CORE-001 | S1～S4 后连续出现正式 `0 request / 112 error`，而公开 baseline 对照不能复现；保守协议 `05ec5c2` 仍为第四次同形失败 | 结构改造与运行策略同时演进，无法用一次正式失败区分入口、client、请求协议、资源或高级阶段影响 | 已保留显式上下文、网关、预算、截断和格式架构，把正式路径收敛为 3 个纯文本候选 + 3 次 verifier，并将物理部署回迁根模块；旧高级开关不能改变调用序列；**线上兼容性与数学能力均待验证** | 首次正式运行只检查 SHA、`request_count>0`、`success>0`、`error=0` 和耗时；成功后再以单变量分支逐项恢复能力，不得一次恢复多个阶段 |
