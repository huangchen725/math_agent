...
P1 已完成严格题型验证接入：`task_router.py` 只为结构明确的直接计算题生成可执行计划；`deterministic_verifier.py` 在隔离子进程中输出三态证据；只有一致的 `pass` 改变选择，其他状态完整回退旧聚合。候选数、温度、thinking mode、模型、模型 verifier 次数和 token 配置均未改变。
