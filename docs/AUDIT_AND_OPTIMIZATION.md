...
> **2026-09-13 新控制器联合重构（离线验收通过）**：依据用户明确执行授权，处理未命中题求解与选答损失。旧流程对公开三重积分的三份互异答案全部给A，并丢失439/120正确候选；该本地反例不能推断为83道正式错题的共同原因。新增实际有界计算、独立路线和具体分歧审查；整题证明、模型子任务、模型审查分层记账。固定外部加载和client边界，历史基线及旧回归保留，新部署单独验收。详见 `evaluations/SOLVER_V2_20260913.md`。

> **2026-09-10晚间首日执行（本地验收通过）**：1054公开完整题答，三项新默认开关启用；保持六源、四参数False、预算与依赖。独立源审核隔离缺参数条件的错误答案，修复快答度数/下标/顺序保真、格式复制与CHECK有界续行，收紧弱参考。同题extra读取共享4MiB/2秒；完整证明才修stop答案交付。真实91请求、139364token；六题初版未命中审核5/6→4/6，暴露原有“多个错误A导致选错”盲点，未声称提分。最终38请求严格基线回放及8确认回放通过，4个快答被独立证据支持；两Python各1114项formal通过。门禁超时通过保判定的Q0局部Counter预筛修复，原60秒与篡改检查保留。详见 [首日报告](evaluations/DAY1_IMPLEMENTATION_20260910.md)。

> **2026-09-10 联合候选实施与验收**：已修复双语路由、全否定答案组处理，接入真实积分题族核验和预算内一次精确失败重算，新增先核算后裁决。普通入口四项明确开启，不再以全关闭默认提交提分试验。两版各三道公开题共 39 请求/48904 token，终答 3/3→3/3，未证明正确率提高；新候选7/9正确，但模型仍错拒正确积分答案，确定性证据保护了终答。最终小修仅忽略裁决间纯空行，全部20条录制请求严格匹配，最终答案行不变。最终两种 Python 各897项门禁通过（46.80/53.10秒），保留60秒限制、原四参数False、8192和导入闭包；独立审查修复的反例全部保留。完整数据、成本增加和剩余边界见[本轮记录](evaluations/ACCURACY_CANDIDATE_20260910.md)。下方同日记录是用户授权联合实施之前的分析状态。

> **2026-09-10 正确率进展复核**：`b87e4b9` 已被正式抓取，25 correct / 85 incorrect / 2 invalid，741 请求、76 截断（10.26%），比昨日少对 5 道。新工程在仓库中，但五项新增能力默认均关闭，默认提示与选答策略仍保持基线；不能把未启用的策略判为正式失效，也不能继续把测试数或关闭开关的工程完成计为提分。下一步优先既有 `Res`/`Express` 路由误判、完整候选到终答的损失分解和实际积分题族核验，不盲目全开或继续扩库。建议与证据边界见 [本次报告](evaluations/OFFICIAL_112_20260910.md)；本轮未实施新的运行时改动或模型实验。

> **2026-09-09 最新正式反馈**：`9acff7e`、输入哈希均与上一轮相同，本次 30 correct / 82 incorrect / 0 invalid，804 请求、78 截断（9.70%）、0 重试，Agent 7,809.925 秒。比同版本上一轮多对 5 道，invalid 从 8 降为 0；代码并未更新，不能归功于第一批新工程或宣称某项修复永久奏效。单次成绩差异不足以判定 A 批稳定提分/退化，截断数也不能直接换算为 invalid 题数。本次仅入档，新策略继续按已有小批证据保持关闭。见 [本次报告](evaluations/OFFICIAL_112_20260909.md)。

> **2026-09-08 第一批 K/C 验收完成**：新增三项独立默认关闭的检索/完整任务核验/条件提示；最终两版本各 773 项全量门禁通过。36 次实际 397B/False 请求完成，27,930 token、0 截断；单阶段正确交付 B1 4→3/4、B2 2→3/8、C2a 5→4/6。检索最终两种语言明确有用均 2/12、C1 开发题完整解析 0/72，全部开关保持关闭，不宣称提分或完整流程验收。两条公开参考争议单独记录，教材一条条件缺失推论已隔离。详见 [第一批完整记录](evaluations/FIRST_BATCH_20260908.md)。下方 B1/B2“未启动真实验证”是此前离线阶段的历史状态。

> **2026-09-08 最新正式反馈**：`9acff7e` 的正式五文件闭包与 `5c2f7a0` 一致，本批 25 correct / 79 incorrect / 8 invalid，112 success / 0 error；截断 50/731（6.84%）。比 `0641043` 少 1 道正确、增加 8 invalid，答案交付增量收益仍未证实；B1/B2 未被本批测到。逐题输出缺失，不能直接认定新提取器或截断是 invalid 原因。入档证据与后续诊断见 [本次报告](evaluations/OFFICIAL_112_20260908.md)。

> **2026-09-08 B2 离线候选**：默认关闭的 `concise_recovery` 只替换补答系统提示，消除“系统要求推导、用户要求直答”的矛盾；512 上限、原用户消息、预算和截断隔离均保留。B1/B2 分别登记，未启动真实验证；旧 `82/91` 补答截断来自未发送 False 的实验，不作为本版本的故障率。

> **2026-09-08 B1 离线候选**：针对纯文本生成仍要求工具调用的确定性提示矛盾，增加默认关闭的 `tool_aware_prompts`，覆盖普通/工具来源及各领域，保留实际工具循环和领域数学内容。新策略不自动进入旧 combined；阶段、配置和原题文本保持可核对。全量验收状态见 [B1/B2 报告](evaluations/B1_B2_OFFLINE_20260908.md)，不将旧默认思考下的失败率视为当前 False 配置的表现。

> **2026-09-08 最新补修**：已核验并同步 `5c2f7a0`，保留正式求解行为。修复离线中继丢弃思考参数、回放忽略参数差异，并兼容旧平铺/新队列录制；严格四参数 fake 保留无私有访问边界。新回归在原版复现 27 失败/42 通过，修复后定向 115 项通过。Python 3.12/3.10 全量 formal 各 666 项通过（42.89/50.91 秒）；详情见 [本轮验收](evaluations/PROTOCOL_REPAIR_20260908.md)。旧 625 项与回放结果属于原三参数版本，不能当成本版本的性能验收。

> 初始审计日期：2026-08-31；最近状态更新：2026-09-07
> 审计对象：当前工作区中的唯一运行链路及其测试、Demo、依赖、文档和交付材料  
> 文档边界：本文件记录问题、目标和实施路线，不定义当前架构。当前架构唯一事实源仍是根目录 `ARCHITECTURE.md`。

> **最新实施状态**：用户授权第一批 A1–A5 离线工程，现已完成。旧响应只读回放、答案块提取、补答完整性传递、本地小批暂停/限额、参考争议材料已实现，Python 3.12/3.10 完整 formal 各 625 项通过（37.47/45.73 秒）。494 份旧响应中 13 份恢复提取、2 份答案体修正、1 份格式讨论被保守拒绝；83 道严格回放有 19 道在分叉处停止，不能推算新的整套正确率。本轮零真实 API；两份参考错误与四类具体争议已进入待独立复核目录，旧标签/分数不变。历史第 45 节的“已发现、未修改”描述保留为当时状态，当前变化与检验结果见 [第一批工程报告](evaluations/ACCURACY_ENGINEERING_IMPLEMENTATION_20260907.md)。

> **恢复期覆盖说明**：第 1～33 节保存 S1～S6 及四次 0 request 调查的历史状态，其中“当前”一词均应按各节日期理解。2026-09-04 起的活动运行时已经恢复到 `350a267f` 内容树；最新结论、底线和重建顺序以第 34 节及 `docs/ENGINEERING_SPECIFICATION.md` 为准。

> **2026-09-05 更新**：恢复锚点已在新平台日志中产生 1069 次请求、112 success / 0 error，答对 24/112；请求链恢复已证实。本次模型已由用户确认为 `intern-s2-preview-35b`；提交页批次和冻结窗口证据待补齐，阶段仍为 R0。最新评测事实以第 36 节为准，之前的“尚无正式非零请求”描述保留为历史状态。
...
## 34. 2026-09-04 同名模块与 client 类型误判事故、恢复定锚和重建决策

扁平化后的 `1fc98b7509b2e07bd229f668cee32e2702cf2cfa` 仍在正式平台得到 0 success、112 error、112 invalid，request/attempt/token 全为 0；Agent 阶段约 15.16 秒。该结果推翻“只要删除 `math_agent/` 子目录即可恢复”的假设。原始日志 SHA-256 为 `68551643cfe8d6defdb356e51a74c31db0bddc169654356d1123cc13450bde68`，完整指标见 `docs/evaluations/OFFICIAL_112_20260904_RUNTIME_FAILURE.md`。

本地按官方 baseline 的真实加载顺序预载外部 `llm_client`，随后加载 `1fc98b7`，得到与正式日志同形的 0 次 `chat()` 和请求前 `AttributeError`。原因是 `model_gateway.py` 的裸导入解析到已缓存的官方类；`isinstance(client, InternChatClient)` 因而错误为真，代码继续访问官方对象没有的 `chat_with_metadata`。仅移除该类型误判和私有方法路径即可恢复 6 次公开请求。独立复核重复得到相同 A/B。

该缺陷对 `1fc98b7` 已形成确定性证据链，但不是此前全部包版本零分的统一解释：`eb5d8d4` 在相同预载碰撞实验中仍可产生 6 次请求。准确结论是，模块拆分和包结构没有被证明是零分的充分原因；真正必须永久禁止的是让通用裸模块名、Python 模块缓存和外部对象类身份共同决定 client 的权限与调用协议。相应规则固化为 `IMPORT-001/002`、`CLIENT-001/002`、`TEST-IMPORT-001`，`OFFICIAL-GAP-CLIENT/ERROR/RUNNER/CHANGE` 继续未解决。

为恢复正式可观测性，当前 `main` 先生成普通前向回退提交，提交 tree 与最后一个官方有分版本 `350a267f` 完全一致；没有执行 `reset --hard` 或强制推送。S1～S6 完整实现保存在 `archive/s1-s6-1fc98b7`。恢复版历史结果为 1082 次请求、23/112、20.54%，但也有约 9.8% 截断和 6 个 invalid，因此它只承担一次兼容性定锚，不能成为最终质量标准。

后续顺序确定为：R0 正式请求定锚；R1 三参数 client、入口、trace、全生命周期和截断的最小契约加固；Q0 建立 100～200 题大学竞赛冻结盲测；Q1 依次验证截断恢复、确定性证据、题型路由、verifier 校准和高级策略；A1 重新引入评测、CI、锁、发布、显式上下文、统一 gateway 和候选/工具分层；Q2 用配对盲审和重复实验验证能力提升。每次正式运行只改变一个变量。

可维护架构可以重新引入。首选形态是根 `user_agent.py` 真实声明入口，正式子模块使用 `xh202627_*` 唯一前缀，单题上下文显式传递，所有模型请求进入不识别 client 类型的三参数 gateway。物理包化放在最后，作为唯一结构变量单独接受正式评测。任何离线结构测试、历史分数或本次 A/B 都不能代替该在线证据。

## 35. 2026-09-04 工作红线触发器与全 Skill 覆盖

为防止规则只存在于长文档、工作 agent 在具体任务中未意识到触线，本轮建立三层触发体系。根 `AGENTS.md` 对每次仓库工作强制生效；`.agents/policies/HARD_RULES.md` 定义事前识别、`[POLICY BLOCK]` 报告格式、安全替代和事后复核；`.agents/policies/policy_manifest.json` 保存恢复阶段、运行时锚点、规则 ID、路径触发面、动作触发面、禁用模式和全部 Skill 覆盖层。`.agents/policy_guard.py` 提供 `--paths`、`--actions`、`--changed`、`--anchor-canary` 和 `--formal` 检查；`--actions` 专门覆盖不产生文件差异的真实 API、推送、正式提交、发布和依赖等动作。

工作 agent 现在必须在修改前列出触发规则，遇到 blocker 时先报告具体动作和后果，再停止触线子动作；不能静默绕过、先执行后说明、删测试或改规则措辞求通过。修改后和提交前必须运行 `--changed`。R0 期间任何运行时改动会触发 `ANCHOR-001`；只有未改变的历史锚点可以通过 `--anchor-canary`，改动版 `--formal` 在阶段进入 R1 前必须失败。

项目维护 Skill 已直接接入该流程。第三方 property-based-testing Skill 保留固定上游提交及各文件原始哈希；项目适配后的文件另记当前哈希和修改说明，并增加本地 `PROJECT_POLICY.md` 覆盖层，限制依赖新增、不安全解析、私有数据和 R0 运行时改动。两个 Skill 的正文与路由提示都会先加载覆盖层，机器清单也要求覆盖层存在，避免只调用第三方 Skill 时绕过项目规则。

规范同步覆盖 README、唯一架构文档、贡献指南、安全说明、竞赛合规清单、官方材料更新规则和提交说明。提交说明中的公开手机号被移除，AtomGit “已同步”改为提交前实测，不再把历史状态冒充当前事实。守卫是本地确定性补充，不能替代官方证据、代码审查、真实 API 授权或正式平台结果。

## 36. 2026-09-05 恢复锚点正式请求链恢复

> 后续独立验收（被审提交 `2cf691f`）：R1 五项“离线全部完成”的声明未获验收通过。现有 103 个测试及静态门禁通过，但独立探针复现宽构造器、trace 脱敏、完整生命周期、截断隔离、导入边界与本地并发 usage 缺口（17 failed、2 个正向对照 passed）。完整复现、严重级别和修复标准见 [R1 离线验收报告](evaluations/R1_OFFLINE_ACCEPTANCE_20260905.md)。本次不撤销进入 R1 的用户决策，不修改运行时或正式成绩。

用户提供的新平台日志绑定 `ba63ac0463fc2afe685b82ac6a1774dabf5027e8`，原件 SHA-256 为 `a8928d0ae66f9e024f23b20c7dec39b5ea1ae8d349a654123c903d00a7057e00`。本次 112 success / 0 error、1069 次请求、1078 次尝试、9 次重试、2864592 token，runner 完成且无基础设施错误。活动运行时与 `350a267f` 的一致性也通过本地 `--anchor-canary` 复核。详见 [本次评测记录](evaluations/OFFICIAL_112_20260905.md)。

判分为 24 correct、81 incorrect、7 invalid，总正确率 21.43%；109 次截断，请求级截断率约 10.20%。用户于 2026-09-05 确认本次模型为 `intern-s2-preview-35b`，日志本身没有模型字段。输入哈希与历史锚点评测一致，但无逐题配对判定，不能把多答对 1 题称为能力提升，也不能把全部 invalid 归因于截断。

R0 的非零请求与成功条件已经有正式日志证据，模型记录已由用户确认补齐；提交页批次回执与冻结窗口记录仍待补充，保留为待补证据项。用户于 2026-09-05 在未提供批次回执的情况下明确指示开始执行 R1，该决策取代“补齐记录后再进入 R1”的原门槛，阶段迁移据此单独办理。R1 按既定顺序加固公开 client 契约、构造、trace、生命周期和截断边界。保留 `OFFICIAL-GAP-CLIENT/ERROR/RUNNER/CHANGE/MODEL`；不得整体恢复归档实现。本次修订仅记录该门槛调整决策，未修改运行时。


## 37. 2026-09-06 R1 缺陷修复与双版本离线验收

用户要求修复独立验收缺陷后，F1–F6 均已实现对应修复并复验。宽构造器忽略不透明附加参数；公开 trace 只保留白名单事件和数值预算；solve 从预检至输出失败关闭；初始、反思和恢复候选统一完整性判定；正式依赖不复用平台同名模块；本地 usage/finish_reason 随请求返回，消除共享最近响应竞争。

原 19 个独立探针已进入 `tests/test_r1_acceptance.py`；并发探针迁移到请求返回值，保持相同交错和 `(11,29)` 断言。另补充 bounded 性质/阶段矩阵和真实 client 回调测试。Python 3.10.11 / 3.12.13 各 187 passed，完整 `--formal` 均通过；正式门禁已包含实际 suite，不再只有静态扫描。详见 [R1 修复与复验记录](evaluations/R1_REPAIR_VALIDATION_20260905.md)。

当前机器无可用 Linux/WSL 环境，第二次正式评测未执行；机器阶段保持 R1，未关闭官方 client/runner/response 等契约缺口。离线通过不代表隐藏集正确率改善或任意文本截断可检测。历史验收否决仍保留为被审提交 `2cf691f` 的事实。本次未推送或点击正式提交。


## 38. 2026-09-06 Q0/Q1 离线交付与严格诊断

按用户截止日决策，在 R1 官方验收待补期间推进 Q0/Q1 离线工作。已准备许可明确的 U-MATH 144 题冻结集（六领域、开发/留出各 72），十组显式 Q1 消融计划，以及完整的来源绑定、四态评分、配对比较、双人审核工作流。真实模型运行及两位实际评审未执行。

严格审核淘汰了存在近重复泄漏的前三版中间题集，修复相似度方向不对称；补齐评分路径/题号/输出指纹校验；确认并修复定义域化简误判、复杂分式括号丢失、符号大小写合并和极端数值资源风险。Q1 的不确定 verifier 不作为 fail，确定性证据只覆盖完整可证明任务，反思开关与 critic 消融分开。

原 187 项与新增回归共 371 项，在 Python 3.12.14 和 3.10.11 全部通过；选定改动模块语句/分支综合覆盖率 86%。Q1 策略默认关闭，模拟测试不代表收益；R1 官方证据、实际 397B 基线、策略收益和人工盲审继续分开记账。详见 [Q0/Q1 离线诊断报告](evaluations/Q0_Q1_OFFLINE_20260906.md)。

## 39. 2026-09-06 正式输出质量退化：512 次截断、52 题 invalid

新日志抓取的是 `c009929`，早于上述 R1 复验修复和 Q0/Q1 提交。112 success / 0 error，但仅 10 correct、50 incorrect、52 invalid；607 请求中 512 截断（84.35%），较 R0 的 10.20% 大幅上升，同一输入哈希下总正确率从 21.43% 降至 8.93%。原件 SHA-256、完整指标和代码诊断见 [正式报告](evaluations/OFFICIAL_112_20260906.md)。

接口三参数投影移除了旧请求的 thinking_mode=False，同时继续沿用 8192/1024/512 输出上限。官方普通 API 文档说明 397B 默认深度思考，但本次日志无模型和思考字段，因此这是优先验证的假设，不能宣称唯一根因。被评测版还存在兜底残句出口和短验证误判风险。当前虽已修复残句出口，仍未实测短预算下的完整答案率；371 项离线测试不能覆盖这一能力缺口。

后续优先开展绑定真实模型与阶段元数据的小规模公开开发集诊断、单变量输出预算试验，再评估其余 Q1 策略。保留 R1 阶段和通用官方契约缺口。本次没有修改运行时或消耗真实 API / 正式评测额度。

## 40. 2026-09-06 截断专项复审：现有 suite 通过，专项验收不通过

按用户全面检查要求完成四版本请求体对照：R0 明确关闭思考，R1 起继承 client / 服务端默认；因此接口投影并非已证明求解行为等价。公开 baseline 普通返回丢弃 finish_reason，正式文本路径不能靠本地 metadata fixture 证明完整截断识别。

当前 `6d42d2c` 新复现：生成阶段预算耗尽丢弃已完成候选（三个反例）；不完整纯文本 verifier 默认按失败并触发额外 critic/reflection；响应 length 可被 metadata stop 掩盖。前两项可能放大高截断下的答案损失，第三项是本地防御缺口，均不能由官方汇总分配实际题数。两个 Python 版本各 384 项全量测试通过，但专项结论仍为不通过，不能据此直接提交。

新增独立真实响应诊断工具，18 请求 / 88064 输出 token 上限，默认离线规划，遇错停止、不重试；真实执行单独等待额度授权。本轮未改正式运行时。逐项证据、复现和应对方案见 [截断专项审计](evaluations/TRUNCATION_AUDIT_20260906.md)。

## 41. 2026-09-06 用户授权截断缺陷修复，执行单次 8192 上限

用户确认 8192 是官方数据并明确要求直接修复。本轮落实单次输出 1–8192 预检与请求边界，修复生成预算耗尽丢失已完成候选、未知验证驱动纠错、stop 覆盖 length，以及明确 length 的空内容错误退出。保留三参数调用、隔离导入、公开 trace 白名单与不复活残句规则。

新增 65 项反例在旧实现 45 failed / 20 passed，修复后通过。两个旧测试原先固定了“生成预算耗尽必须丢弃已有答案”，已按明确修复目标改为完整答案保留且请求数不增加；失败历史保留。真实诊断计划改为全部不超过 8192，18 次 / 63488 输出 token 上限，旧计划不可执行。真实截断率、思考模式因果和正式成绩仍待实测，详见 [修复验收](evaluations/TRUNCATION_REPAIR_20260906.md)。

最终 Python 3.12.14 和 3.10.11 完整 formal 各 452 项通过，Ruff、编译和计划一致性检查通过。已复现代码缺陷的离线验收通过；真实 API 未执行，不关闭模型/平台截断原因与实际收益验收。

## 42. 2026-09-07 Q2 离线证据流程补齐

原 Q0/Q1 已有单次配对与评分工具，但缺少运行前登记、重复实验统计、完整候选矩阵约束、候选锁定及留出使用状态。Q2 新增对应离线工作流，修复可能重复计数同次运行、事后丢弃不利批次、变更输出仍沿用验收、成本缺失被当作免费收益等工程缺口；未知判分通过最不利区间保留，不强行猜对错。

每题多次运行先按题汇总再重采样，不扩大独立样本量；保留 invalid、状态迁移、请求/尝试/token/延迟及截断记录。模拟结果不能晋级。留出使用标记和指纹只能约束本地工作流，不能证明独立未见、真实提供方身份或防止拥有全部文件权限者伪造。

公开 U-MATH 144 题方案已冻结，真实运行 0，留出未启用；独立 fixture 六次运行完成。Python 3.12.14 和 3.10.11 完整 formal 各 505 项通过，Ruff、编译和政策守卫通过。正式运行时未改，R1 与模型实测仍待验收。完整工程结果和限制见 [Q2 离线报告](evaluations/Q2_OFFLINE_20260907.md)。

## 43. 2026-09-07 正式 26/112、invalid=0；下一阶段转向正确率

新日志抓取 `0641043`，26 correct / 86 incorrect / 0 invalid，803 请求、77 次截断（9.59%），总 token 1,505,458，Agent 墙钟 6363.181 秒。相对上一异常批次，多对 16 题、invalid 少 52 题；相对最早基线仅多对 2 题，不能宣称稳定数学能力提升。三次输入指纹相同，但没有逐题配对、实际模型回执及本次源码，不能做确定归因。

当前本地及 GitHub main 是 `35fbea6`。用户仅判断此次大概率是 Q1 完成、Q2 未完成版本；后续使用当前可追溯代码开展公开开发集实验，不把它称为同版 26 分基线。Q2 只增加评测工程，不能据其是否完成解释正式成绩。

用户明确下阶段目标为正确率，已冻结 baseline/compact_routing 首轮 72 题配对；当前仅准备方案，真实调用 0，未使用留出。先检查实际 final_response 正确率，再按失败类型决定候选多样性、verifier、确定性证据和反思消融；保持已有可靠性边界。详见 [正式报告](evaluations/OFFICIAL_112_20260907.md) 和 [提分计划](evaluations/ACCURACY_NEXT_20260907.md)。

## 44. 2026-09-07 首轮实测中止与离线判分误判

用户随后明确授权首轮最多 2304 次请求并要求等待完整结果。大小写严格比较误拒一次 `Intern-S2-Preview-397B` 回执后，保留原研究并在剩余额度内新开替代批次；替代批次第 37 次请求约 120.136 秒后 `ReadTimeout`，基线仅完成 6/72，候选组未开始。两批累计 38 次尝试、37 份成功回执、已知 53,566 token，另一次失败用量未知。实际辅助执行器无 HTTP 重试且禁止重定向，按原方案遇不确定传输错误即停止；该超时没有 finish_reason，不能计为已确认的 8192 截断。读取等待未给既有服务时限口径预留余量，180 秒读取/230 秒中转等待方案已零网络验证，尚未重开。

离线旧 judge 对六题给出 4 unknown / 2 wrong，其中两个 wrong 都是正确答案的等价写法。根因是无条件采用运行时 canonical 对未知多项元素的字面比较；扩展反例同时暴露顺序交换误判正确、集合重复元素误判错误。只修复离线 judge，未知集合/多项进入复核，显式精确数值有限集合使用集合语义，增加字符/元素边界且不进入不可信递归或符号解析。正式运行时和策略保持冻结，不能声称其所有语义风险都已消除。

专项旧实现 11 failed / 11 passed；修复后相关 137 项通过，Python 3.12 / 3.10 完整 formal 各 522 passed（33.82 / 40.45 秒），原 60 秒门限未改。Ruff、53 个仓库 Python 文件编译、few-shot 零请求预检通过。保留旧分数与全部 checkpoint，修复后六题均 unknown；独立保存本 assistant 的逐题数学核对，六题答案均成立，但不是双人盲审或整体正确率。

判分摘要变更正确使旧 Q1/Q2 研究失效，没有绕过旧凭证/完整矩阵检查。新计划需明确同意重开，仍受原累计请求上限约束，未使用留出、未推送或提交。详见 [首轮实测与后续方案](evaluations/ACCURACY_PILOT_20260907.md)。

## 45. 2026-09-07 用户批准重开后的旁路诊断（现已按用户要求暂停）

用户明确同意重开，并授权等待最多延长至 360 秒。v3 选择 HTTP 读取 300 秒、中转等待 350 秒，继续原累计 2304 次尝试上限，历史 38 次及未知用量不清零。已观察到约 222、239、271 秒的正常回执；这证明旧 120 秒等待窗口会丢失部分慢响应，不能证明输出截断已经消除。当前 Q1 源码摘要 `fab3c040e9e16371c61a1ac0c11d14e3f96978e1f74618dac4b363c1ea2020c1` 保持冻结。

用户随后明确要求因运行太慢先暂停。已终止离线控制器，写入停止标记，收齐暂停前已发出的最后一笔回执并确认网络进程退出；没有继续启动新题或自动续跑。基线 72 题完整，对照组 11 题完整，第 12 题未完成；这是一轮中的第二种策略，不是第二次重复。v3 共 494 次请求且全部有回执，累计含历史为 532 次、已知 1,153,518 token，历史一笔未知用量保留。最后一笔在控制器退出后返回，其成本由外部收尾追加登记并注明来源；原账本、状态及部分运行摘要保存为 `.before-user-pause` 文件。`user-pause-receipt.json` 和 `audited-result.json` 记录暂停与核对结果，未把部分对照组登记为完整 Q2 运行。

运行中只做离线旁路核查，以下缺陷仍是“已发现、未修改运行时”：

- 多行答案提取丢失：完整响应的最终答案标记与编号列表分行时可能被拒绝，实际导致第 8、13 题“未解出”。通用反例还表明分行小题可只提取第一项，末尾空答案标记可回取旧答案，先前 boxed 中间值可抢占末尾多行答案块。后续修复必须同时保留顺序、完整性和截断拒绝边界。
- 加粗答案标记被误作幂运算：第 38 题的 `**最终答案：**` 结束标记被带入答案，再从 `**` 归一化成开头的 `^`。零 API 样例 `**最终答案：** 7` 同样产生 `^ 7`，属于整理过程引入的污染；修复时需同时保护合法 `x**2` 幂语法。数学核查保留该题的交付格式歧义，不清理原答案后强行计对。
- 工具提示与公开文本调用能力不一致：当前没有本地工具适配器，却仍要求模型调用工具。第 30 题三份长候选均正常 stop，但都停在工具调用文本上，随后恢复失败；普通候选的领域系统提示也包含工具要求。第 35 题甚至出现模型自行编写、先错后改的“工具响应”，实际 tool_calls=0，不能归因为计算器执行错误。
- 恢复提示互相冲突：恢复的系统消息要求思路和推导，用户消息要求只给最终答案。第 34 题响应 197 明确花费输出讨论这一冲突；其三次 512 token 恢复均未完成。短恢复和验证请求的截断比例必须与 8192 解题请求分别统计，不把所有失败归为网络等待。
- 符号恢复答案在内部传递时丢失标记：`_quick_fallback` 返回公式本体，候选聚合再次要求答案标记，导致完整的 `sqrt(2)` 或第 51 题实际恢复的 `\tan^2(\alpha)` 被隔离。零网络重放同时确认裸数字 `7` 正常。全部候选失败后的最终兜底直接构建回复，须与每个候选的恢复路径区分；后续修复应保留已验证的答案结构，不能放宽所有无标记残文。
- 正常 stop 也有空正文：请求 269、288 的 completion 为 1655、3256，保存的正文却仅有换行。现有辅助脚本只保留 `message.content` 或原生工具消息，没有保存完整 HTTP JSON，不能确认是否存在其他推理字段；这两次不能归类为 length 截断或读取超时。冻结运行期间没有为补日志而更改执行器。
- 单题预算仍在请求边界检查：第 37 题三份长候选均 length，最后一次返回时已耗时 626797 ms，下一次恢复被预算拒绝，实际为“未解出”。没有完整候选时无法保留答案；这与第 7 题完整候选保留成功、第 15 题保留完整但数学错误的候选，是不同的结果。
- 公开参考标签存在确定错误：第 28 题对数内根号常数应为 5，冻结标签写为 1。取 `x=-1/5`，模型交付式导数和题面被积函数是 `9/(5*sqrt(5))`，参考式导数为 `9/5`。第 66 题实数参数收敛集为 `[-1,1]`，参考 `p<=1` 接受反例 `p=-2`，其项不趋于零；模型最终给出正确的 `|p|<=1`。原题集、自动评分均保留，另存精确反例；其他几何解释或方法要求的歧义只作备注，不据此宣称参考必错。
- 最终答案、正文正确性和题目误差要求需分开核验：第 64、66 题最终公式正确，但模型声称工具确认的系数或数值表有错，实际工具调用仍为零。第 70 题的最终小数 `0.7468` 与参考不完全相同，精确交错级数界却证明其误差小于 `3.61e-5`，满足题目 `1e-4` 要求；冻结自动判分因原标签含文字而记 unknown，不是一次已发生的自动 wrong。数学审核不覆盖原分数，也不把最终答案正确称作全文无错。

逐题核查绑定实际交付答案和 checkpoint SHA，只作为单次 assistant 数学诊断；模型残文、工具调用文本中的正确表达式不补算为求解成功。自动四态评分、该诊断与真实双人盲审分别记录。完成本次两组前不作完整配对结论；首轮结果也不能替代 Q2 重复矩阵或独立留出证据。详见 [运行报告](evaluations/ACCURACY_PILOT_20260907.md) 和忽略目录中的原始回执、`assistant-review.json`、`reference-label-issue-28.json`。
