...
当前项目还没有达到“可靠数学求解器”的标准。项目报告记录的 112 题隐藏集水平约为 22%，历史峰值为 28.57%；当前只有 3 道公开样例，无法证明跨 18 领域的稳定正确率。第一层保守改动已经补上结构化答案、预算、工具硬超时、usage 和运行摘要，S0+S1 又把唯一运行实现迁入 `math_agent/` 包，但都没有改变求解策略。当前主要瓶颈不是 Agent 数量，而是以下五项：

1. 缺少能代表真实题型分布的固定回归集；
2. 确定性验证已接入严格直接题，但对证明、复合目标和带约束任务覆盖仍窄；
...
| 模型验证 | 严格直接题先做隔离的确定性验证，其余仍有独立 verifier 视角 | 证明和语义题的 verifier 与求解器同源，不能提供独立真值 | P1 |
| 答案处理 | 已有 `Answer` 类型及数值、集合、多解的保守 canonical key | 区间和复杂符号等价覆盖仍有限 | P1 |
| 聚合选择 | 一致确定性 `pass` 优先；无证据、失败、未知或冲突时回退多数票和置信度 | P1 真实增益尚未完成匿名 A/B；不可验证题仍受多数票相关错误影响 | P0 |
...
| 安全隐私 | 密钥忽略、工具白名单、路径安全；公开提交文档已移除手机号 | trace 可能泄露题面；无 secret scan | P0 |
...
- `math_agent/answer_equivalence.py`：保守等价判定；
...
6. 建立 CI、锁文件、secret scan、许可证决策和可复现打包。
