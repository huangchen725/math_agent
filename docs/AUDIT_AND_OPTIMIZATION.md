...
R0 的非零请求与成功条件已经有正式日志证据，模型记录已由用户确认补齐；提交页批次回执与冻结窗口记录仍待补充，本次未迁移机器阶段。后续补齐记录后单独进入 R1，按既定顺序加固公开 client 契约、构造、trace、生命周期和截断边界。保留 `OFFICIAL-GAP-CLIENT/ERROR/RUNNER/CHANGE/MODEL`；不得整体恢复归档实现。本次只修改证据文档，没有修改运行时、调用模型、提交或推送。
