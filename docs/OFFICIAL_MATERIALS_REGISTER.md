...
> **2026-09-05 追加证据**：用户提供的新平台日志绑定 `ba63ac0`，记录 1069 次请求、112 success / 0 error、24/112 correct，恢复锚点的正式请求链已获正证据。用户随后确认本次模型为 `intern-s2-preview-35b`；提交页批次回执及冻结窗口仍待补齐，保留为待补证据项。模型确认补充本次实验记录，不构成通用模型映射或版本冻结保证，不关闭 `OFFICIAL-GAP-CLIENT/ERROR/RUNNER/CHANGE/MODEL`。用户另于 2026-09-05 在未提供批次回执的情况下明确指示开始执行 R1，阶段迁移按该决策单独办理；此为项目决策记录，不是官方证据。详见 [本次评测记录](evaluations/OFFICIAL_112_20260905.md)。
...
| MAT-015 | 用户转交的平台评测日志（2026-09-08） | `eval_log_98a0427534104cefb78e1c248954616f.log`；抓取 `9acff7ea8fa946a68b845e636761eed9b1646b51` | `9e72dd6387e576b399929ba8bfe4b7d53573d97c368343fc683a2f8995ab8ccc` | 731 请求、50 截断、112 success / 0 error、25 correct / 79 incorrect / 8 invalid；无逐题输出和模型/思考回执 |
| MAT-016 | 用户转交的平台评测日志（2026-09-09） | `eval_log_8f7fee5cfdb74a7bab5bfee87e8b531a.log`；抓取 `9acff7ea8fa946a68b845e636761eed9b1646b51` | `c99ee2003bcaa125794caab71855b30c50fed8bb891edfd3f708b194b56ee958` | 同提交同输入哈希再次观测：804 请求、78 截断、0 重试、112 success / 0 error、30 correct / 82 incorrect / 0 invalid；仍无逐题输出和模型/思考回执 |
| MAT-017 | 用户转交的平台评测日志（2026-09-10） | `eval_log_3a8a668a27bf4ea9bf279d4bc3ab5fc1.log`；抓取 `b87e4b9f2f7383960c659b887c39daa809f228cb` | `7da30dc860ff36df40995dba536833a1fd00587901217b8173af1168f2a7f6a4` | 新工程合并提交的运行观测：741 请求、76 截断、6 重试、112 success / 0 error、25 correct / 85 incorrect / 2 invalid；无逐题答案或实际构造/模型/思考回执 |
| MAT-018 | 用户转交的平台评测日志（2026-09-11） | `eval_log_a03e713ebd344180989e061f0f13ca89.log`；抓取 `43a6a1478c300ef1f94d68915a8bc9bb47333179` | `fde43ea8dec5d45769b8bff3d59ad8852431ba9b335953bd3948cbeaa57f31ab` | 689请求、73截断、20重试、112 success / 0 error、28 correct / 77 incorrect / 7 invalid；尚未测到后续扩库版，无逐题与实际模型/配置回执 |
| MAT-019 | 用户转交的平台评测日志（2026-09-12） | `eval_log_1ef62e27e6ec4a7eb56a0e039d9abbae.log`；抓取 `1841685526e6f4d9c7211fb867d4db0aa65155d4` | `45da061daa1caeea5b848c74d491469e418b41fdb0648f5bb97a28d4c24817f2` | 794请求、93截断、1重试、112 success / 0 error、27 correct / 83 incorrect / 2 invalid；被测源码含扩库快答批次，未提供实际命中次数及模型/构造回执 |
...
MAT-015 补充说明（2026-09-08）：输入 SHA 与前三批相同，准确率 22.32%，请求级截断 6.84%，Agent 10923.374 秒、13 次重试。源码核对 `9acff7e` 的正式五文件闭包与 `5c2f7a0` 一致，均保留 False；与 `0641043` 的正式差异仅为入口 +114/-16 行交付加固。B1/B2 尚未进入本批。没有逐题判分和实际请求回执，不能将 8 invalid 归因于具体修复、50 次截断或模型；不关闭通用 `OFFICIAL-GAP-*`。详见 [本次正式报告](evaluations/OFFICIAL_112_20260908.md)。

MAT-004 同版本补充核验（2026-09-08，检索与报告边界）：重新读取上述固定 SHA 的 README。赛题简介明确鼓励检索与记忆；实现自由度明确禁止读取或构造隐藏测试集、标准答案或 judger 信息，并禁止依赖固定题序和跨题状态。“日志与评测报告”明确下载产物不包含逐题题面、答案、候选、原始交互或判分细节；因此不再把等待用户下载逐题材料作为当前诊断前提。允许检索不自动证明“命中公开题库后无推理直接返回库存答案”这一细分做法得到明确许可。MAT-009 离线 RAG 口径保留为 2026-09-01 的核验记录，本次飞书页面读取失败，不更新其核验日期，不关闭其它官方缺口。具体评估见 [公开语料检索提案](evaluations/CORPUS_RETRIEVAL_PLAN_20260908.md)。

MAT-016 补充说明（2026-09-09）：原件 26,044 字节，Agent 墙钟 7,809.925 秒，总 token 1,506,365；两份独立汇总与 output_json 副本一致。与 MAT-015 使用相同完整提交和输入 SHA，正确题数 +5、invalid -8，但截断增加到 9.70%。这是同版本的结果差异，不能证明由模型随机性、judge、服务或环境中哪一项引起，也不能归功于未进入被测提交的 B1/B2/第一批 K/C。源码保留 False 不等于服务端执行回执；不据本次 0 invalid 关闭任何通用契约缺口。详见 [本次正式报告](evaluations/OFFICIAL_112_20260909.md)。

MAT-017 补充说明（2026-09-10）：原件 26,044 字节，输入 SHA 与前五批相同，Agent 11,030.226 秒，总 token 1,483,782，双份独立汇总与 output_json 副本一致。源码已是合并后的 `b87e4b9`，因此昨日 GitCode 尚旧的远端观测不能解释本批；正常默认构造下 B1/B2/检索/精确核验/条件检查均未开启。该源码事实不冒充平台实际配置回执。新六源文件闭包完成本次运行，但成绩仍未体现稳定提升，不据单次差值或运行成功关闭通用缺口。详见 [本次报告](evaluations/OFFICIAL_112_20260910.md)。

MAT-018 补充说明（2026-09-11）：原件25980字节，输入SHA与此前六批一致；Agent14041.149秒，总token1361154，各汇总副本及加和复算通过。被测43a6a14的普通源码构造已启用四项联合策略及双语路由，并非全False；后续1054条答案库和快答交付批次未进入该提交。本地55f1799的全部684个Git对象与已验收包一致，但本轮未查远程，不能据此判定当前比赛仓库已同步。此运行观测不证明各策略增益，不将7个invalid对应到73次请求截断，不关闭通用OFFICIAL-GAP-*。详见 [本次报告](evaluations/OFFICIAL_112_20260911.md)。

MAT-019 补充说明（2026-09-12）：原件26047字节，独立汇总与output_json副本一致；输入SHA与此前七批派生回执相同，Agent8945.315秒，总token1666770。旧原件本轮已不在原微信路径，对照复用先前已核验归档，不冒充重新读取八份原件。1841685与55f1799仅差三份文档，代码与资源、六源验收摘要一致，七项部署开关在普通源码构造中开启。本批不能再以新代码未同步解释，但也无法从汇总确认实际资源加载、命中、快答接受或单项贡献；不将提速直接归功于查表，不关闭通用OFFICIAL-GAP-*。详见 [本次报告](evaluations/OFFICIAL_112_20260912.md)。
