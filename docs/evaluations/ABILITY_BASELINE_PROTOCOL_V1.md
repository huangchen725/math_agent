...
P0 的离线基础设施和真实基线均已完成。当前状态是 `baseline_executed_pending_candidate_blind_review`：冻结旧版已经在公开基准上完成 120 题 × 3 次真实调用，可靠性门禁通过；数学正确率仍未盲审，不能把当前 `unknown` 结果解释为 0% 或 100%，也不能提前声明新版能力已经提升。

冻结的运行时基线为 commit `831b94f1f0e2e57a4cf93e5c4b3f588c89f0d5cb`。本轮只增加评测、盲审和配对统计工具，没有修改 `ReasoningAgent`、领域提示、候选数、温度、工具轮数或聚合规则。

## 已执行基线结果（2026-08-30）

三轮均使用数据 SHA-256 `f5f7666651b4a8b4214d6fae7a1a24e1caa4b816cb8b0d2f3a12b2a40c57f0d7`、配置模型 `intern-s2-preview`、本地并发 1 和冻结运行时 commit `831b94f1f0e2e57a4cf93e5c4b3f588c89f0d5cb`。所有 360 个题目运行均为 `success`，没有 runner error、missing 或 invalid。

| 轮次 | 模型请求 | 截断 | 请求级截断率 | 候选生成截断率 | 恢复覆盖 | invalid |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| 1 | 1,064 | 25 | 2.35% | 3.65% | 25 / 25 | 0 |
| 2 | 1,063 | 31 | 2.92% | 4.37% | 31 / 31 | 0 |
| 3 | 1,046 | 25 | 2.39% | 3.48% | 25 / 25 | 0 |
| 合计 | 3,173 | 81 | 2.55% | 3.83% | 81 / 81 | 0 |

合计请求级截断率的单侧 95% Wilson 上界为 3.06%，候选生成截断率的同口径上界为 4.63%，均低于 5%。共有 64 个“题目 × 轮次”发生过截断，64 个最终都保留了有效答案；截断残句进入最终答案为 0。短恢复直接成功 69 次，另外 12 次未直接成功，但均被隔离并由后续兜底保住有效输出。三轮共使用 7,176,832 tokens，累计题级耗时约 7 小时 31 分钟。

这组数据只证明冻结旧版的运行可靠性和资源基线。120 个题目的三轮结果目前都保持 `unknown`，应在候选版本完成后进行同题匿名 A/B 盲审，避免先看旧版答案再形成不对称判分标准。完整无题面摘要见 [基线执行结果](ABILITY_BASELINE_RESULTS_V1.md)。

有一项已知溯源限制：客户端收到了接口响应中的模型字段，但旧版预算 trace 未持久化该字段，因此本次只能证明配置模型名、端点配置和冻结代码一致，不能从产物反证平台实际返回的模型标识。若正式声明要求响应级模型身份，应先补充脱敏持久化，再按同一协议重跑所需版本。
...
- 基线运行已经获得授权并完成；候选版本运行前仍必须再次确认真实 API 费用和请求额度。本协议文档本身不调用 API。
