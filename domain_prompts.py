...
XXX 只写答案本体，不写“答案是”、解释或完整句子；若已有精确形式，不要只写小数近似值；能等价表示时优先使用 ASCII 记号（如 x^2、C1、Z），复杂公式可用 LaTeX。
"""
...
【Few-shot】
题：求 u_tt=c²u_xx 在 [0,L] 上 u(0,t)=u(L,t)=0, u(x,0)=f(x), u_t(x,0)=0 的解。
解：分离变量 u=X(x)T(t)，得 X''+λX=0, T''+c²λT=0。
边界条件 → X_n=sin(nπx/L)，λ_n=(nπ/L)²。
T_n=A_n cos(nπct/L)+B_n sin(nπct/L)，零初速度给出 B_n=0。
u=Σ A_n cos(nπct/L)sin(nπx/L)，其中 A_n 由 f(x) 的傅里叶正弦展开确定。
