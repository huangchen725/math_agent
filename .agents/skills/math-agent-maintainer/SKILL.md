...
- Preserve the current conservative formal sequence: three plain-text candidate calls followed by one compact verifier call per eligible candidate, for exactly six normal requests on a well-formed solve. The four recovery requests are reserved for truncation or the whole-problem emergency path. Legacy candidate/verifier counts and tools/critic/reflection/deterministic fields are compatibility inputs only and must not alter the sequence or enable another formal protocol.
...
For injected-client changes, send unknown clients exactly `messages`, `temperature`, and `max_tokens`. Project-private atomic metadata is permitted only behind the project client's nominal type boundary; current formal callers still submit only the three public request arguments. Never read a marker from an injected object. Retain a fake with an incompatible same-name method, a fake that raises on private attribute access, and a minimum-signature fake whose `chat` has no `**kwargs`. Assert the exact six-call sequence and that legacy advanced flags cannot call private methods or add request kwargs.
