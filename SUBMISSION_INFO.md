...
- 发榜单位：上海人工智能实验室
- 队伍名称：XH-202627基于Intern-S1的数学智能体设计与推理创新
- 负责人姓名：钟奕鸣
- 联系电话：13826011382
...
## 系统简介
本智能体基于官方 baseline（InternLM/Challenge-Cup-2026）增强，采用"规划-生成-验证-反思-聚合"五阶段架构：

1. **题型识别与领域路由**：规划阶段识别 18 个数学子领域，注入领域专属专家 prompt
2. **工具增强求解**：通过 OpenAI 兼容 tool_calling 调用 SymPy 工具（求根/求导/积分/留数/极限），精确计算消灭算术错误
3. **多候选生成**：工具增强候选 + 纯推理候选混合，温度采样增加多样性
4. **验证投票**：独立验证器多轮投票，VERDICT 机制评估候选正确性
5. **反思纠错**：验证不通过时反馈重试一轮
...
| math_tools.py | SymPy 数学计算工具（6个工具 + tool_calling 循环） |
...
- 每题平均 trace 步数：14 步（含规划、工具调用、验证、聚合）
