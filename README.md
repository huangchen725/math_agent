...
> **当前开发候选（2026-09-10）**：按用户授权联合推进双语路由、完整公开题诊断与数学核验。普通注入入口使用 `deployment_policy()`，实际开启简洁补答、有界数学核验、证据选答和先核算后裁决；全候选精确失败时预算内追加至多一次定向重算。显式 `Q1Policy()` 仍全关闭，但新路由始终生效，严格旧基线须用冻结的旧源码。保持四参数 `thinking_mode=False`、8192 上限及原请求/时间预算。尚无本轮正式提分结果，详见 [联合候选记录](docs/evaluations/ACCURACY_CANDIDATE_20260910.md)。
